...
use iced::{
    ...
    widget::{
        ...
        Column, Container, PickList, Row, Text,
    },
...
impl TablesUI {
    ...
    pub fn content<'a>(&'a self) -> Element<'a, Message> {
        ...
        if self.show_create_table_form {
            row = row.push(self.tables_component_section(
                ...
                <TablesUI as UIComponent>::EventType::ShowOrRemoveCreateTableForm.message(),
            ));
        }
        ...
        if let Some(table_info) = &self.single_table_info {
            row = row.push(self.tables_component_section(
                table_info.content(),
                <TablesUI as UIComponent>::EventType::UndisplayTableInfo.message(),
            ));
        }

        if self.show_single_table_data {
            row = row.push(self.tables_component_section(
                self.single_table_data.content(),
                <TablesUI as UIComponent>::EventType::ShowOrRemoveTableData.message(),
            ));
        }
    ...
    fn tables_component_section<'a>(
        &'a self,
        content: Element<'a, Message>,
        undisplay_message: Message,
    ) -> Container<'a, Message> {
        let mut component_section = Column::new().spacing(10).padding(10);
        component_section = component_section.push(content);

        let undisplay_button = button("🔙 Back")
            .style(|_, _| button_style())
            .on_press(undisplay_message)
            .padding(10);

        component_section = component_section.push(undisplay_button);
