use crate::components::business_components::{
    component::{
        BColumn, BColumnForeignKey, BConstraint, BDataType, BTableChangeEvents, BTableGeneral,
...
impl UIComponent for TableInfoUI {
    ...
    fn update(&mut self, message: Self::EventType) -> Task<Message> {
        match message {
            ...
            Self::EventType::ResetTableInfo => {
                self.active_foreign_key_dropdown = None;
                let table_info = self.table_info.clone();
                let table_name = self.table_name_display.clone();
                Task::perform(
                    async move {
                        table_info.set_table_info(table_name).await;
                    },
                    |_| Self::EventType::SetTableInfo.message(),
                )
            }
            Self::EventType::SetTableInfo => {
                self.table_name_display = self
                    .table_info
                    .table_name
                    .blocking_lock()
...
impl TableInfoUI {
    ...
    pub fn content<'a>(&'a self) -> Element<'a, Message> {
        ...
        table_info_column = table_info_column
            ...
            .push(self.reset_table_info_button());
    ...
    fn render_foreign_key_button<'a>(
        ...
        let button = button(button_text)
            .style(|_, _| toggle_foreign_key_dropdown_button_style())
    ...
    fn update_table_button(&self) -> Button<'_, Message> {
        button("🛠️ Update Table")
            .style(|_, _| update_button_style())
            .padding(10)
            .on_press(TableInfoMessage::SubmitUpdateTable.message())
    }

    fn reset_table_info_button(&self) -> Button<'_, Message> {
        button("Reset Table Info")
            .style(|_, _| reset_button_style())
            .padding(10)
            .on_press(TableInfoMessage::ResetTableInfo.message())
    }
}
...
fn reset_button_style() -> button::Style {
    button::Style {
...
fn toggle_foreign_key_dropdown_button_style() -> button::Style {
    button::Style {
        background: Some(Background::Color(Color::from_rgb(0.0, 0.6, 0.9))), // CRM blue button
        border: Border {
            color: Color::from_rgb(0.0, 0.4, 0.7),
...
fn update_button_style() -> button::Style {
    button::Style {
        background: Some(Background::Color(Color::from_rgb(0.9, 0.0, 0.0))), // Red button
        border: Border {
            color: Color::from_rgb(0.7, 0.0, 0.0), // Darker red for the border
            width: 2.0,
            radius: Radius::from(5.0),
        },
        text_color: Color::WHITE,
        shadow: Shadow {
            color: Color::from_rgb(0.0, 0.0, 0.0), // Black shadow
            offset: Vector::new(0.0, 2.0),
            blur_radius: 5.0,
        },
    }
